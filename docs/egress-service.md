...
- `routingTable`: The routing table to use for the service.
Both the numeric id of a routing table and its name are supported as values.
The table will be used for the egress traffic of the Service's endpoints and on return traffic for when an endpoint replies to an external client calling the service.
When the field is not specified the default routing table of the host is used.
...
### Routing Table
In addition, an `EgressService` supports a `routingTable` field.
...
Assuming an `EgressService` has `routingTable: blue`, a ClusterIP of 10.96.135.5 and its endpoints are 10.244.0.3 and 10.244.1.6, a `fwmark` will be internally assigned to the "blue" table and the following will be created on the host:
