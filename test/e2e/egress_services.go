...
var _ = ginkgo.Describe("Egress Services", func() {
	...
	ginkgo.BeforeEach(func() {
		...
		nodes = n.Items
		ginkgo.By("Creating the external containers to send the traffic to/from")
...
`)
	...
	ginkgotable.DescribeTable("[LGW] Should validate pods' egress uses Network without SNAT",
		func(protocol v1.IPFamily, dstIP *string) {
			ginkgo.By("Creating the backend pods")
			podsCreateSync := errgroup.Group{}
			for i, name := range pods {
		...
		ginkgo.BeforeEach(func() {
			...
			for _, net := range []*netSettings{net1, net2} {
				...
				net.containerIPv4, net.containerIPv6 = createClusterExternalContainer(net.containerName, agnhostImage,
					[]string{"--privileged", "--network", net.name, "--hostname", net.name}, []string{"netexec", fmt.Sprintf("--http-port=%d", podHTTPPort)})
...
`)
					...
					// comment blabla arbitrary node we don care
				...
				for _, net := range []*netSettings{net1, net2} {
					for i, pod := range net.createdPods {
						...
						gomega.Consistently(func() error {
							return curlAgnHostHostnameFromPod(f.Namespace.Name, pod, net.name, dst, podHTTPPort)
				...
				ginkgo.By("Deleting the EgressServices the backend pods should not be able to reach the client (no routes)")
