...
var _ = ginkgo.Describe("Egress Services", func() {
	...
	ginkgo.BeforeEach(func() {
		...
		externalIPv4, externalIPv6 = createClusterExternalContainer(externalContainerName, agnhostImage,
			[]string{"--privileged", "--network", externalContainerNetwork}, []string{"netexec", fmt.Sprintf("--http-port=%d", podHTTPPort)})
	})
	...
	ginkgotable.DescribeTable("Should validate pods' egress is SNATed to the LB's ingress ip without selectors",
		func(svcIP string, dstIP *string) {
			...
			for _, pod := range pods {
				...
				}, 3*time.Second, 500*time.Millisecond).ShouldNot(gomega.HaveOccurred(), "failed to reach external container with loadbalancer's ingress ip")
			}
...
func curlAgnHostClientIPFromPod(namespace, pod, expectedClientIP, dstIP string, containerPort int) error {
	...
	if err != nil {
		return fmt.Errorf("failed to curl external container from %s, err: %w", pod, err)
	}
	ourip, _, err := net.SplitHostPort(out)
	if err != nil {
		return fmt.Errorf("failed to split agnhost's clientip host:port response, err: %w", err)
	}
	if ourip != expectedClientIP {
		return fmt.Errorf("reached external container with ip %s from %s instead of %s", ourip, pod, expectedClientIP)
	}
	return nil
}

func reachAllServiceBackendsFromExternalContainer(container, svcIP string, svcPort int32, podsToReach []string) {
	backends := map[string]bool{}
	for _, pod := range podsToReach {
		backends[pod] = true
	}

	dst := net.JoinHostPort(svcIP, fmt.Sprint(svcPort))
	for i := 0; i < 10*len(podsToReach); i++ {
