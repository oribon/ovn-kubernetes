...
type Controller struct {
	...
	availableFWMark int32  // available fwmark for a routing table that needs one
...
func NewController(stopCh <-chan struct{}, thisNode string,
...
func (c *Controller) syncEgressService(key string) error {
	...
	fwmark, err := c.fwmarkForTable(es.Spec.RoutingTable)
	if err != nil {
		return err
	}
	...
	for ip := range marksToAdd {
		err := nodeipt.AddRules([]nodeipt.Rule{fwmarkIPTRuleFor(key, cachedState.fwmark, ip)})
		if err != nil {
			return err
		}
