...
func (c *Controller) repair() error {
	...
	for _, es := range egressServices {
		...
		v4, v6, err := c.allEndpointsFor(svc, es.Status.Host == noSNATHost)
		if err != nil {
...
func (c *Controller) syncEgressService(key string) error {
	...
	v4Eps, v6Eps, err := c.allEndpointsFor(svc, es.Status.Host == noSNATHost)
	if err != nil {
		return err
	}
...
func (c *Controller) shouldConfigureEgressSVC(svc *corev1.Service, svcHost string) bool {
	return (svcHost == c.thisNode || svcHost == noSNATHost) &&
